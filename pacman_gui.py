...
class MainWindow(QMainWindow):
    def __init__(self, qtable_path: str = QTABLE_PATH):
        ...
        self.speed_slider.setToolTip("Simulation speed")
        ...
        speed_row.addWidget(self.lbl_speed_value)
    ...
    def _speed_to_interval_ms(self, speed_value: int) -> int:
        # 1 -> slow (200ms), 100 -> fast (1ms)
        slowest = 200
        fastest = 1
        return int(slowest - ((speed_value - 1) * (slowest - fastest) / 99.0))

    def on_speed_changed(self, speed_value: int):
        interval = self._speed_to_interval_ms(int(speed_value))
        self.timer.setInterval(interval)
        self.lbl_speed_value.setText(f"{int(speed_value)}% ({interval}ms)")
    ...
    def update_status_panel(self):
        ...
        self.lbl_status.setText(
            ...
            f"speed: {self.speed_slider.value()}% ({self.timer.interval()}ms/tick)\n"
            f"policy: {mode}\n"
