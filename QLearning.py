...
class World:
    ...
    def step(self):
        ...
        s = self.get_state()
        s_existed_before = s in self.agent.Q

        # Snapshot for one-step undo.
        self._undo_snapshot = {
            "robot": (self.robot.x, self.robot.y, self.robot.heading_rad),
            "pills": [(p.x, p.y, p.is_green) for p in self.pills],
            "steps": self.steps,
            "score": self.score,
            "last_reward": self.last_reward,
            "last_reward_train": self.last_reward_train,
            "last_action": self.last_action,
            "collected_green": self.collected_green,
            "hit_red": self.hit_red,
            "reward_block_sum": self._reward_block_sum,
            "reward_block_count": self._reward_block_count,
            "reward_ma_1000": self.reward_ma_1000,
            "reward_ma_history": list(self.reward_ma_history),
            "reward_ma_dirty": self.reward_ma_dirty,
            "eps": self.agent.eps,
            "state": s,
            "state_existed_before": s_existed_before,
        }
        ...
        s_next_existed_before = s_next in self.agent.Q

        self._undo_snapshot["state_next"] = s_next
        self._undo_snapshot["state_next_existed_before"] = s_next_existed_before
        self._undo_snapshot["q_sa_before"] = self.agent.Q[s][a]
        self._undo_snapshot["action"] = a
    ...
    def undo_last_step(self) -> bool:
        """Undo exactly one previous world.step(), including its Q-update."""
        snap = self._undo_snapshot
        if snap is None:
            return False

        self.robot.x, self.robot.y, self.robot.heading_rad = snap["robot"]

        for pill, (x, y, is_green) in zip(self.pills, snap["pills"]):
            pill.x = x
            pill.y = y
            pill.is_green = is_green

        self.steps = snap["steps"]
        self.score = snap["score"]
        self.last_reward = snap["last_reward"]
        self.last_reward_train = snap["last_reward_train"]
        self.last_action = snap["last_action"]
        self.collected_green = snap["collected_green"]
        self.hit_red = snap["hit_red"]

        self._reward_block_sum = snap["reward_block_sum"]
        self._reward_block_count = snap["reward_block_count"]
        self.reward_ma_1000 = snap["reward_ma_1000"]
        self.reward_ma_history = list(snap["reward_ma_history"])
        self.reward_ma_dirty = snap["reward_ma_dirty"]

        self.agent.eps = snap["eps"]

        s = snap["state"]
        s_next = snap["state_next"]
        a = snap["action"]
        q_before = snap["q_sa_before"]
        s_existed_before = bool(snap["state_existed_before"])
        s_next_existed_before = bool(snap["state_next_existed_before"])

        if s in self.agent.Q:
            self.agent.Q[s][a] = q_before

        # Remove rows that only existed because of the undone step.
        if (not s_existed_before) and (s in self.agent.Q):
            if not (s == s_next and s_next_existed_before):
                del self.agent.Q[s]

        if (not s_next_existed_before) and (s_next in self.agent.Q) and (s_next != s):
            del self.agent.Q[s_next]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.btn_undo = QPushButton("Rueckgaengig")
